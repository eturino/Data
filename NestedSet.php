...
class EtuDev_Data_NestedSet
{
    ...
    static public function getSelectPath($tableObject, $id)
    {
        ...
        $select->join(
            array('n' => $tableName),
                'n.left_ BETWEEN p.left_ AND p.right_ ' . ($id > 1 ? 'AND p.level > 0' : ''),
            array()
        ); //level > 0 para quitar ROOT, siempre que no estemos pidiendo root
...
//		$categories = $this->db->GetAll('SELECT * FROM ' . $this->table . ' WHERE left_ BETWEEN \'' . ($row['left_'] + 1) . '\' AND \'' . $row['right_'] . '\'' . (0 < $levels ? 'AND level BETWEEN ' . ($row['level'] + 1) . ' AND ' . ($row['level'] + $levels) : '') . ('' . ' ' . $where . ' ORDER BY left_'));
        ...
        $select->join(
            array('p' => $tableName),
                'n.left_ BETWEEN p.left_ AND p.right_ ' . ($include_self ? '' : 'AND n.id != p.id ') . ($only_direct_children ? 'AND n.level = p.level + 1' : ''),
            array()
        ); //level > 0 para quitar ROOT
