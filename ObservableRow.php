...
class EtuDev_Data_ObservableRow extends Zend_Db_Table_Row_Abstract implements EtuDev_Interfaces_ToArrayAbleFull
{
    ...
    */
                ...
                try {
                    ...
                    static::log(
                        'ObservableRow/notifyObserver',
                            'Fallo al notificar: ' . $event . ' de: ' . $this->getTableClass() . ' Error: ' . $e->getMessage(),
                        Zend_Log::ERR
                    );
                }
            }
    ...
    protected function _loadClassInfo()
    {
        ...
        $this->_aliases             = $keyscont ? array_merge(
            array_combine($keyscont, $keyscont),
            (array) $info['aliases']
        ) : (array) $info['aliases'];
    ...
    protected function _get($origkey)
    {
        ...
        if ($getter) {
            ...
            $key = $this->_transformColumn(
                $origkey
            ); //nos hemos asegurado en los sets y en la carga de info que todos los definidos en container están en alias, por lo que podemos hacer esto (es más rápido que comprobar) Mejoramos la velocidad de GET con respecto a la de SET (se hace siempre muchas más veces el GET)
    ...
    public function toArray($level = null, $toArrayToArrayables = false)
    {
        ...
        if ($level == self::LEVEL_ALL) {
            ...
            $reals = @$this->_properties_by_level[$level] ? : (array_key_exists(
                $level,
                $this->_properties_by_level
            ) ? array() : @$this->_properties_by_level[self::LEVEL_ALL]);
        }
        if (!$reals) {
            return array();
        }
